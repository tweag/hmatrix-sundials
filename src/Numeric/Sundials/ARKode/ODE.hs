...
arkOdeC :: CConsts -> CVars (VS.MVector RealWorld) -> ReportErrorFn -> IO CInt
arkOdeC CConsts{..} CVars{..} report_error =
  [C.block| int {
  ...
  */
  ...
  while (1) {
    ...
    flag = ARKStepEvolve(arkode_mem, ($vec-ptr:(double *c_sol_time))[i], y, &t, ARK_NORMAL); /* call integrator */
